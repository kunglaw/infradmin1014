...
/**
 * Class Log_agentsea
 * Handle logging of agentsea.
 *
 * @author pulung
 * @copyright 2015 PT. Badr Interactive
 */

class Log_agentsea extends CI_Controller {

    private $_primary_table = "log_agentsea";
    private $_agentsea_table = "perusahaan";

    private $_menu = MENU_AGENTSEA;

    private $_route = "agentsea/log";
    private $_parent_route = "agentsea";
    private $_view_folder = "log_agentsea";
    ...
    public function list_item($item_id) {
        ...
        $item = $this->generic->retrieve_one(
            ...
            array("id_perusahaan" => $item_id)
        );

        if (empty($item)) {
            set_notification("Agentsea does not exist", NOTIF_ERROR);
            redirect($this->_parent_route);
        }

        $data["item_name"] = $item["nama_perusahaan"];
    ...
    public function get_list_item_ajax($item_id) {
        ...
        $this->datatables->where($this->_primary_table .".agentsea_id", $item_id);
