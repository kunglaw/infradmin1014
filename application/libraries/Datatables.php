...
class Datatables
{
    ...
    public function __construct()
    {
		...
		// $this->set_database(DB_GROUP);
		/* echo "<script>alert('".$this->table."')</script>"; */
		/* if (!$this->ci->db->table_exists($this->table) && $this->table != "") {
            // kunglaw edit
			$this->set_database(DB_GROUP);
			if(!$this->$this->ci->db->table_exists($this->table))
			{
				return NULL;
			}
		...
		// KUNGLAW  EDIT
		// load dahulu bro 
		$urii = $this->ci->uri->segment(1);
		if($urii == "role" || $urii == "admin")
		{
			$this->set_database("default");
		}
		else
		{
			$this->set_database(DB2_GROUP);
		}
    }
     ...
     */
    public function set_database($db_name = "infr6975_2014")
    {
    ...
    public function from($table)
    {
		...
		if (!$this->ci->db->table_exists($table)) {
            // kunglaw edit
			$this->ci->db = $this->load->database(DB2_GROUP, TRUE);
			if(!$this->$this->ci->db->table_exists($table))
			{
				return NULL;
			}
	...
	// kunglaw edit
	/* public function flex_edit_column($column,$content,$match_replacement)
	{
		$this->edit_columns[$column][] = array('content' => $this->load->view($content,true), 'replacement' => $this->explode(',', $match_replacement));
        return $this;
	}*/
