    ...
    <div class="modal-content">
      ...
      <form method="post" action="" role="form" id="form-account-type" >
        <div class="modal-header bg-header-modal">
          ...
          <h4 class="modal-title"><?=$agentsea['nama_perusahaan']?> - Change Account Type</h4>
          ...
          <div class="col-md-5">
           <h4> Account Type </h4>
            <div class="form-group">
              <select name="account_type" id="account_type" class="form-control" style="width:90%">
              	<?php
				if($agentsea['account_type'] == "Free_trial")
				{
					$selected_free_trial = "selected=selected";	
				}
				else if($agentsea['account_type'] == "Free")
				{
					$selected_free = "selected=selected";	
				}
				else if($agentsea['account_type'] == "Premium")
				{
					$selected_premium = "selected=selected";	
				}
				else if($agentsea['account_type'] == "Premium_advance")
				{
					$selected_premium_advance = "selected=selected";	
				}
				
				?>
                <option value="Free_trial" <?=$selected_free_trial?> >Free Trial</option>
              	<option value="Free" <?=$selected_free?> >Free </option>
                <option value="Premium" <?=$selected_premium?> >Premium</option>
                <option value="Premium_advance" <?=$selected_premium?> >Premium Advance </option>
...
function submit_account_type()
{
	$.ajax({
		...
		url:"<?=base_url("agentsea/account_type_process")?>",
		// error:function(dt)
		// {
